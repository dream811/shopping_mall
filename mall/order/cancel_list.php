...
-->
     ...
     <br>
			<?
			$sql = "select idx from wiz_basket where status != '' and (status = 'CA' or status = 'CI' or status = 'CC') and mallid = '".$wiz_mall['id']."'";
			...
			$sql = "select * from wiz_basket where (status = 'CA' or status = 'CI' or status = 'CC') and mallid = '".$wiz_mall['id']."' $status_sql $period_sql $searchopt_sql $reason_sql order by ca_date desc";
			$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
        ...
        <?
       	...
       	<?
			...
			<tr><td height="5"></td></tr>
			 <tr>
			   <td width="30%">
					<a class="AW-btn" onClick="basketDelete();">선택삭제</a>
					<a class="AW-btn" onClick="batchStatus();">상태 일괄변경</a>
			   </td>
