...
//-->
    	...
    	$sql = "select wb.idx from wiz_bbs as wb left join wiz_product as wp on wb.prdcode = wp.prdcode
    					where wb.code='$code' and wb.prdcode != '' and wp.mallid = '".$wiz_mall['id']."'";
      ...
      $all_total = mysqli_num_rows($result);
			...
			$sql = "select wb.idx from wiz_bbs as wb, wiz_product as wp where wb.code='$code' and wb.prdcode = wp.prdcode and wp.mallid = '".$wiz_mall['id']."' $search_sql order by wb.wdate desc";
			$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="right">
          <table cellspacing="0" cellpadding="0" border="0" class="t_style">
          ...
          <td class="t_value">
			  ...
			  <input type="text" name="keyword" value="<?=$keyword?>" size="13" class="input">
      ...
      <?
			$sql = "select wb.*, from_unixtime(wb.wdate) as wdate, wp.prdimg_R, wp.prdname from wiz_bbs as wb, wiz_product as wp where wb.code='$code' and wb.prdcode = wp.prdcode and wp.mallid = '".$wiz_mall['id']."' $search_sql order by wb.wdate desc limit $start, $rows";
			$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
      ...
      ?>
        ...
        <tr>
          ...
          <td align="center"><a onclick="delEstimate('<?=$row->idx?>');" class="AW-btn-s">삭제</a></td>
        </tr>
      ...
      <table width="100%" height="10" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td width="33%"><a onClick="delSelEstimate();" class="AW-btn">선택삭제</a></td>
