...
$now_position = "<a href=/>Home</a> &gt; 커뮤니티 &gt; <strong>".$bbs_info['title']."</strong>";
...
<div class="AW_ttl clearfix">
	...
	<div class="his_bar clearfix">
		<a href="/" class="home_ico">Home</a><span>고객센터</span>
	</div>
</div>
...
if($wpermi >= $mem_level && $code != "qna") $write_btn = "<a href=".$bbs_info['input_url']."?code=$code&mallid=$mallid&mode=write><img src=/images/bbsimg/btn_write.gif border=0></a>";

// 버튼설정
$list_btn = "<a href='".$bbs_info['list_url']."?code=$code&mallid=$mallid'>리스트</a>";
if($wpermi >= $mem_level) {
	$write_btn = "<a href='".$bbs_info['input_url']."?mode=write&code=$code&mallid=$mallid' class='btn'>글쓰기</a>";
} else {
	...
	while($row = mysqli_fetch_array($result)) {
	    ...
	    if($category == $row['idx']) {
			...
			if(!strcmp($row['gubun'], "A")) {
				$catlist .= "<a href='$PHP_SELF?ptype=list&code=".$code."&mallid=$mallid'>".$catname."</a>";
			} else {
	    	$catlist .= "<a href='$PHP_SELF?ptype=list&code=".$code."&mallid=$mallid&category=".$row['idx']."'>".$catname."</a>";
	    }
...
while($row = mysqli_fetch_array($result)){
	...
	$no = "<font color=red>[공지]</font>"; $name = $row['name']; $wdate = $row['wdate']; $count = $row['count'];
	if($row['comment'] > 0) $comment = "(".number_format($row['comment']).")";
	else $comment = "";
	$subject = "<a href='".$bbs_info['view_url']."?idx=".$row['idx']."&page=$page&$param'>".$row['subject']."</a>";
...
$sql = "select idx from wiz_bbs where code = '$code' and notice != 'Y' $my_sql $category_sql $search_sql order by prino desc";
...
$sql = "select wb.*,wb.wdate as wtime,from_unixtime(wb.wdate, '".$bbs_info['datetype_list']."') as wdate, wc.catname, wc.caticon
				from wiz_bbs as wb left join wiz_bbscat as wc on wb.category = wc.idx
				where wb.code = '$code' and wb.notice != 'Y' $category_sql $search_sql $my_sql
...
while($row = mysqli_fetch_array($result)){
	...
	if(!empty($row['upfile1'])) {
		// 첨부파일
		$file_icon = "<a href='/bbs/down.php?code=$code&mallid=$mallid&idx=".$row['idx']."&no=1'><img src='$skin_dir/image/file.gif' border='0' align='absmiddle'></a>";			// file
	}
	...
	$subject = "<a href='".$bbs_info['view_url']."?idx=".$row['idx']."&page=$page&$param'>".$row['subject']."</a>";		// subject
	$viewBbs = "".$bbs_info['view_url']."?idx=".$row['idx']."&page=$page&$param";
	if($row['privacy'] == "Y"){																																					// privacy
		if(
			...
			$subject = "<a href='".$bbs_info['auth_url']."?mode=view&idx=".$row['idx']."&page=$page&$param'>".$row['subject']."</a>";
			$viewBbs = "".$bbs_info['auth_url']."?mode=view&idx=".$row['idx']."&page=$page&$param";
			if(!empty($file_icon)) $file_icon = "<a href='".$bbs_info['auth_url']."?mode=view&idx=".$row['idx']."&page=$page&$param'><img src='$skin_dir/image/file.gif' border='0' align='absmiddle'></a>";
		}
