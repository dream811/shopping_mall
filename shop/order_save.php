...
//상품에 입점업체 등록이 안되어 있으면 계산이 안되는 문제 innerjoin을 leftjoin으로 수정
...
$sql = "SELECT wb.*, wm.del_method, wm.del_fixprice, wm.del_staprice, wm.del_staprice2, wm.del_staprice3, wm.del_prd, wm.del_prd2,
				...
				FROM wiz_basket_tmp as wb left join wiz_mall as wm on wb.mallid = wm.id
				WHERE uniq_id='".$_COOKIE["uniq_id"]."' $idx_sql";
...
// 주문상품 저장
//상품에 입점업체 등록이 안되어 있으면 계산이 안되는 문제 innerjoin을 leftjoin으로 수정
$sql = "SELECT wb.*, wp.del_type, wp.del_price FROM wiz_basket_tmp as wb left join wiz_product as wp on wb.prdcode = wp.prdcode WHERE wb.uniq_id='".$_COOKIE["uniq_id"]."' $idx_sql";
...
if(mobile_check() == true) {
 $go_url = "http://".$_http_host."/".$mobile_path."/sub/order_pay.php?orderid=".$orderid."&pay_method=".$pay_method;
} else {
