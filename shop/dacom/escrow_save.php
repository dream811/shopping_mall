...
$sql = "insert into wiz_test(idx,memo,wdate) value('','test',now())";
mysqli_query($connect, $sql);
    ...
    $mertkey = $pay_key; //데이콤에서 발급한 상점키로 변경해 주시기 바랍니다.
...
$sql = "insert into wiz_test(idx,memo,memo2,wdate) value('','test','$mid.$oid.$tid.$txtype.$productid.$ssn.$ip.$mac.$resdate.$mertkey',now())";
mysqli_query($connect, $sql);
