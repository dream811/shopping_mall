...
function Calendar1(obj, target) {													// jucke
...
function Calendar2(obj, target, top, left) {									// jucke
	...
	minical.style.pixelTop = top + "px";
	minical.style.pixelLeft = left + "px";
	minical.style.display = (minical.style.display == "block") ? "none" : "block";
...
function Calendar3(obj, target1, target2, target3) {														// jucke
	...
	minical.style.pixelTop = (y + 12) + "px";
	minical.style.pixelLeft = (x - 20) + "px";
	minical.style.display = (minical.style.display == "block") ? "none" : "block";
...
function doClick() {															// 날자를 선택하였을 경우
	var now;
	cal_Day = window.event.target.title;
	window.event.target.style.borderColor = "red";							// 테두리 색을 빨간색으로
...
function Show_cal(sYear, sMonth, sDay) {
	...
	Cal_HTML = "<TABLE WIDTH=100% CELLPADDING=0 CELLSPACING=0 ONMOUSEOVER=doOver(); ONMOUSEOUT=doOut(); STYLE='font-size:8pt;font-family:Tahoma; border:1mm ridge rgba(170, 50, 220, .6);'>"
		...
		for (intLoopDay = 1; intLoopDay <= 7; intLoopDay++) {						// 요일단위 루프 시작, 일요일 부터
			if (intThirdWeekday > 0) {											// 첫주 시작일이 1보다 크면
				...
				if (thirdPrintDay > intLastDay) {								// 입력 날짝 월말보다 크다면
					...
					Cal_HTML += "<TD onClick=doClick(); title=" + intThisYear + "-" + day2(intThisMonth).toString() + "-" + day2(thirdPrintDay).toString() + " STYLE=\"cursor:pointer;border:1px solid white;font-weight:bold;";
