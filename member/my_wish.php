...
<div class="AW_ttl clearfix">
	...
	<div class="his_bar clearfix">
		<a href="/" class="home_ico">Home</a><span>관심상품</span>
	</div>
</div>
...
<form action="<?=$PHP_SELF?>" method="get">
	...
	<span class="count">등록되어 있는 관심상품은 총 <span class="11red_01"><strong><?=$total?></strong></span>개 입니다.</span>
...
<tbody>
 <tr>
	<th height="35" width="5%" align="center">선택</th>
	<th align="center" colspan="2">관심상품정보</th>
	<th width="15%" align="center">판매가</th>
	<th width="15%" align="center">저장일시</th>
</tr>
  <?
	$sql = "select ww.*, wp.prdcode, wp.prdname, wp.sellprice, wp.strprice, wp.prdimg_R, wp.popular, wp.recom, wp.new, wp.sale, wp.shortage, wp.stock, wp.conprice
	from wiz_wishlist ww, wiz_product wp
	where ww.memid = '".$wiz_session['id']."' and ww.prdcode = wp.prdcode $order_sql limit $start, $rows";
	$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
	 ...
	 <tr>
		...
		<input type="hidden" name="idx" value="<?=$row->idx?>">
			...
			<td style="padding-left:10px;" class="con">
				<a href="/shop/prd_view.php?prdcode=<?=$row->prdcode?>&catcode=<?=$catcode?>&page=<?=$page?>" target="_blank"><img src="<?=$row->prdimg_R?>" border="0" width="60"></a>
			</td>
			<td style="text-align:left; padding-left:12px;">
				<p style="font-size:16px; line-height:1.5;"><a href="/shop/prd_view.php?prdcode=<?=$row->prdcode?>&catcode=<?=$catcode?>&page=<?=$page?>" target="_blank" style="display:inline-block; margin-right:5px; vertical-align:middle;"><?=$row->prdname?></a><?=$sp_img?></p>
				<?=$optcode?>
			</td>
   ...
   <?
	...
	if($total <= 0){
	?>
	<tr><td align="center" colspan="10" style="height:200px;">관심상품 리스트가 비어있습니다.</td></tr>
	<?
	}
	?>
</tbody>
...
<div class="AW_btn_area clearfix" style="margin:10px auto 20px">
	<button type="button" class="list_btn" onClick="delPrd();">선택삭제</button>
	<button type="button" class="list_btn" onClick="basketPrd();">장바구니 담기</button>
</div>
